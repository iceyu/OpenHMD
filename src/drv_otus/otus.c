...
#define OTUS_SENSORS_PID 0x0659
...
typedef struct {
	...
	uint32_t last_ticks;
	uint8_t last_seq;
    otus_sensors_packet sensor;

} otus_priv;

static void vec3f_from_OTUS_gyro(int16_t smp[3][32], int i, vec3f* out_vec)
{
...
static void vec3f_from_OTUS_accel(int32_t smp[3][4], int i, vec3f* out_vec)
{
...
static void handle_tracker_sensor_msg(otus_priv* priv, unsigned char* buffer, int size)
{
    ...
    otus_sensors_packet* s = &priv->sensor;
	...
	for(int i = 0; i < 4; i++){
		...
		vec3f_from_OTUS_gyro(s->gyro, i, &priv->raw_gyro);
		vec3f_from_OTUS_accel(s->accel, i, &priv->raw_accel);
			       ...
			       unsigned char* buf, int len)
{
	...
	do {
		...
		if (size == -1)
			return -1;
		if (buf[0] == OTUS_IRQ_CONTROL)
			return size;
	} while (buf[0] == OTUS_IRQ_SENSORS || buf[0] == OTUS_IRQ_DEBUG);

	return -1;
}

int read_config_part(otus_priv *priv, unsigned char type,
...
unsigned char *read_config(otus_priv *priv)
{
...
static ohmd_device* open_device(ohmd_driver* driver, ohmd_device_desc* desc)
{
	...
	// Open the HMD device
	priv->hmd_imu = open_device_idx(MICROSOFT_VID, OTUS_SENSORS_PID, 0, 1, idx);
	...
	// turn the IMU on
	hid_write(priv->hmd_imu, otus_sensors_imu_on, sizeof(otus_sensors_imu_on));
...
{
	struct hid_device_info* devs = hid_enumerate(MICROSOFT_VID, OTUS_SENSORS_PID);
	struct hid_device_info* cur_dev = devs;
